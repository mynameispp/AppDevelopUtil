...
//                    return new QQLockView(context);
        ...
        mPaint.setStrokeWidth(DeviceUtil.dp2px(context, mStrokeWidth));
    ...
    private void setLockViewParams(LockViewFactory lockViewFactory) {
        ...
        for (int i = 0; i < mDotCount * mDotCount; i++) {
            ...
            //设置圆点颜色，可以不设置，有默认色
            ((JDLockView)mILockViews.get(i).getView()).setNoFingerColor(noFingerColor);
            ((JDLockView)mILockViews.get(i).getView()).setFingerTouchColor(fingerTouchColor);
            ((JDLockView)mILockViews.get(i).getView()).setUnmatchedColor(unmatchedColor);
    ...
    public void setPathWidth(float dp) {
        mPaint.setStrokeWidth(DeviceUtil.dp2px(getContext(), dp));
    }
